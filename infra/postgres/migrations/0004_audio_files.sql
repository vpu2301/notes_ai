-- 0004 — `audio_files`: the row that catalogues every recorded-audio
-- object stored in MinIO.
--
-- The actual bytes live in MinIO (encrypted via the envelope, no plaintext
-- ever at rest — recordings are sensitive personal data). This row stores
-- the metadata required for downstream jobs, retention, audit, and GDPR
...
-- ── Grants ───────────────────────────────────────────────────────────
-- DELETE is deliberately NOT granted: retention/erasure is a privileged
-- operator flow, never an app_role statement (rows soft-delete via
-- status='deleted' + crypto-shredding of the object).
