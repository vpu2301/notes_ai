...
async def list_synonym_groups(
    ...
    return [
        SynonymGroupOut(group_id=g.group_id, source=g.source, language=g.language, terms=g.terms)
        for g in groups
    ]
...
async def update_synonym_group(
    ...
    if not replaced:
        ...
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="tenant synonym group not found")
    await state.audit_writer.write_event(
        tenant_id=claims.tid,
