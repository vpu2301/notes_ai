...
class BoundNote(_Strict):
    """Content-free: the caller holds template.update, not note.read."""
...
async def create_template(
    ...
    state = get_state()
    async with tenant_connection(state.app_pool, claims.tid) as conn:
        new_id = await repository.create_template(conn, tenant_id=claims.tid, definition=body)
...
async def deprecate_template(
    ...
    if outcome == "in_use":
        raise HTTPException(
            ...
            detail=(
                "templates referenced by draft notes cannot be deprecated; "
...
@router.get(
    "/{template_id}/bound-notes",
    response_model=list[BoundNote],
    summary="Content-free list of notes bound to a template (admin re-bind UI).",
)
async def list_bound_notes(
    ...
    async with tenant_connection(state.app_pool, claims.tid) as conn:
        ...
        if template is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
        rows = await repository.list_bound_notes(conn, template_id=template_id, limit=limit)
    return [
        BoundNote(
            note_id=r["id"],
            status=r["status"],
...
_REBIND_ERRORS: dict[str, tuple[int, str]] = {
    ...
    "note_not_found": (status.HTTP_404_NOT_FOUND, "note not found"),
    "not_bound": (
        status.HTTP_409_CONFLICT,
        "note is not bound to this template",
    ),
    "not_draft": (
        status.HTTP_409_CONFLICT,
        "only draft notes can be re-bound; finalized and amended notes keep their template",
    ),
    "same_template": (
        status.HTTP_409_CONFLICT,
        "note is already bound to this template",
    ),
...
@router.post(
    ...
    summary="Move one draft note to a successor template (audited).",
)
async def rebind_note(
    ...
    state = get_state()
    async with tenant_connection(state.app_pool, claims.tid) as conn:
        outcome = await repository.rebind_note(
            conn,
            template_id=template_id,
