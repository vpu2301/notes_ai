...
def compute_diff(
    *,
    ...
    from_content: NoteContent,
    to_version_id: str,
    to_version_number: int,
    to_content: NoteContent,
) -> DiffResponse:
...
def _metadata_diff(a: NoteContent, b: NoteContent) -> MetadataDiff:
    return MetadataDiff(
...
def section_diff_summary(diff: DiffResponse) -> dict[str, list[str]]:
    """Compact representation suitable for ``note_versions.diff_jsonb``."""
    return {
