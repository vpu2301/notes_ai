...
``mdx_notes_autosave_rate_limit_hits_total`` increments.
"""
...
class AutosaveRateLimiter:
    ...
    async def check_and_record(self, *, note_id: UUID) -> tuple[bool, int]:
        """Returns (allowed, retry_after_seconds)."""
        async with self._lock:
            slot = self._slots[note_id]
            now = time.monotonic()
    ...
    def reset(self, note_id: UUID) -> None:
        self._slots.pop(note_id, None)
