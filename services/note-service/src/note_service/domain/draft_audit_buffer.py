...
class DraftAuditBuffer:
    ...
    async def record(
        ...
        *,
        ...
        key = (tenant_id, note_id, dictation_session_id)
    ...
    async def flush_session(
        self, *, tenant_id: UUID, note_id: UUID, dictation_session_id: UUID
    ) -> None:
        key = (tenant_id, note_id, dictation_session_id)
        async with self._lock:
        ...
        if entry is not None:
            try:
                await self._flush_fn(tenant_id, note_id, dictation_session_id, entry)
