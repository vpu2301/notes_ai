...
def validate_content_metadata(
    content: NoteContent, template: TemplateDefinition
        ...
        if tpl_section is None:
            problems.append(
                MetadataProblem(
                    ...
                    reason="section is not defined in the note's template",
                )
            )
            continue
        try:
...
async def check_content_metadata(conn: object, *, content: NoteContent) -> list[MetadataProblem]:
...
async def template_field_types(conn: object, *, content: NoteContent) -> dict[str, str]:
    ...
    losing it must not fail a user's autosave.
    """
    try:
        tmpl_row = await repository.get_template(conn, template_id=content.template_id)
        if tmpl_row is None:
