"""Note synthesis seam (spec item 1).

A :class:`Synthesizer` turns the *raw dictation* of a single note
...
Invariants the mock guarantees (relied on by the unit tests and by
the content-safety posture):
...
* **No invention** — it only normalises whitespace, capitalises sentence
  starts and trims; it never adds, drops, or reorders note content.
...
class AnthropicSynthesizer:
    ...
    to rewrite raw dictation into clean prose. That sends note content
