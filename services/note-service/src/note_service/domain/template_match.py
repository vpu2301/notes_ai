...
# Fallback preference when nothing scores: the general meeting-notes template.
FALLBACK_CATEGORY: Final = "general"
...
def _stem(token: str) -> str:
    ...
    «презентація» → «презентаці» matches «презентації»;
...
async def load_candidates(conn: asyncpg.Connection, *, language: str) -> list[TemplateCandidate]:
    ...
    rows = await conn.fetch(
        """
        SELECT id, code, name, category, schema_version, schema_jsonb, is_system
        FROM templates
...
def select_template(candidates: list[TemplateCandidate], transcript: str) -> TemplateChoice | None:
    """Best-scoring candidate, or the general meeting-notes fallback.
    ...
    # Fallback order: the meeting-notes template ("meeting_notes*"
    # code), then any general template, then whatever exists.
    fallback = next(
        (c for c in candidates if c.code.startswith("meeting_notes")),
        next(
            (c for c in candidates if c.category == FALLBACK_CATEGORY),
            candidates[0],
        ),
    )
