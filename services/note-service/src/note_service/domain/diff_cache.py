...
``(note_id, from_id, to_id)``. Production deployments can swap to a
...
class DiffCache:
    ...
    def get(self, *, note_id: UUID, from_id: UUID, to_id: UUID) -> DiffResponse | None:
        key = (note_id, from_id, to_id)
    ...
    def put(self, *, note_id: UUID, from_id: UUID, to_id: UUID, value: DiffResponse) -> None:
        key = (note_id, from_id, to_id)
