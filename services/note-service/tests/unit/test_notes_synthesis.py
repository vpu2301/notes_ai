"""Note synthesis endpoints + mock engine (spec item 1).

No infra: the auth dep is overridden and the DB/audit boundary is stubbed
with an in-memory job store (mirrors ``test_notes_section_labels``).
"""
...
RAW_FINDINGS = "  the   deadline has [[slipped]] again.  no   blockers"
CLEAN_FINDINGS = "The deadline has [[slipped]] again. No blockers"
...
def _content() -> NoteContent:
    return NoteContent(
        ...
        sections=[
            NoteSection(section_key="findings", text=RAW_FINDINGS),
            NoteSection(section_key="impression", text="stable"),
        ],
    )
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    from note_service.domain import synthesis_jobs
    from note_service.main import create_app
    from note_service.routers import notes_synthesis
    ...
    monkeypatch.setattr(notes_synthesis, "tenant_connection", _fake_tenant_conn)
    ...
    monkeypatch.setattr(notes_synthesis.repo, "fetch_note", _fetch_note)
    monkeypatch.setattr(notes_synthesis.repo, "fetch_version", _fetch_version)
    ...
    monkeypatch.setattr(notes_synthesis.repo, "append_version", _no_append)
    ...
    async def _find_completed(conn, *, note_id, request_hash):  # noqa: ANN001
        job_id = by_hash.get((note_id, request_hash))
    ...
    async def _insert(
        ...
        *,
        ...
        by_hash[(note_id, request_hash)] = job_id
        return job_id

    async def _fetch_job(conn, *, job_id, note_id):  # noqa: ANN001
        job = by_id.get(job_id)
        return job if job and job.note_id == note_id else None
    ...
    notes_synthesis.get_synthesizer.cache_clear()
...
def test_post_synthesize_returns_original_and_clean_text(client: TestClient) -> None:
    resp = client.post(f"/v1/notes/{NOTE_ID}/synthesize", json={"language": "en"})
    ...
    assert "[[slipped]]" in by_key["findings"]["text"]
    # Audit: started + completed both emitted.
    kinds = [c["kind"] for c in client.audit_calls]  # type: ignore[attr-defined]
    assert "note.synthesis_started" in kinds
    assert "note.synthesis_completed" in kinds


def test_synthesize_is_idempotent(client: TestClient) -> None:
    first = client.post(f"/v1/notes/{NOTE_ID}/synthesize", json={"language": "en"})
    second = client.post(f"/v1/notes/{NOTE_ID}/synthesize", json={"language": "en"})
    assert first.status_code == second.status_code == 200
    assert first.json()["job_id"] == second.json()["job_id"]


def test_get_synthesis_job_returns_stored_result(client: TestClient) -> None:
    post = client.post(f"/v1/notes/{NOTE_ID}/synthesize", json={"language": "en"})
    job_id = post.json()["job_id"]
    got = client.get(f"/v1/notes/{NOTE_ID}/synthesize/{job_id}")
...
def test_synthesize_does_not_mutate_note(client: TestClient) -> None:
...
def test_mock_synthesizer_deterministic_and_preserves_markers() -> None:
    from note_service.domain.synthesis import MockSynthesizer
...
def test_anthropic_synthesizer_is_a_stub() -> None:
    from note_service.domain.synthesis import AnthropicSynthesizer, build_synthesizer
