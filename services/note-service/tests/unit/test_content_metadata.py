...
from note_service.domain import content_metadata
from note_service.domain.content_metadata import (
    check_content_metadata,
    validate_content_metadata,
)
from note_service.routers._content_guard import ensure_valid_field_metadata
...
def _content(sections: list[dict[str, Any]]) -> NoteContent:
        ...
        [
            {
                ...
                "text": "email",
                "field_specific_metadata": {"selected": ["email"], "source": "manual"},
            }
        ]
    )
    assert validate_content_metadata(content, _template()) == []
...
def test_empty_metadata_everywhere_passes() -> None:
    content = _content([{"section_key": "summary", "text": "гарна розмова"}])
    assert validate_content_metadata(content, _template()) == []
...
def test_unknown_selected_value_flagged() -> None:
        ...
        [
            {
                "section_key": "deal_stage",
                "text": "",
                "field_specific_metadata": {"selected": "won", "source": "manual"},
            }
        ]
    )
    ...
    assert "won" in problems[0].reason
        ...
        [
            {
                "section_key": "deal_stage",
                "text": "",
                "field_specific_metadata": {"selected": "lead"},  # no source
            }
        ]
    )
    problems = validate_content_metadata(content, _template())
        ...
        [
            {
                "section_key": "summary",
                "text": "x",
                "field_specific_metadata": {"selected": "lead", "source": "manual"},
            }
        ]
    )
    problems = validate_content_metadata(content, _template())
...
async def test_fast_path_skips_template_fetch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    content = _content([{"section_key": "summary", "text": "x"}])
...
async def test_guard_raises_422_with_section_addressed_problems(
    ...
    assert extras["problems"][0]["section_key"] == "deal_stage"
