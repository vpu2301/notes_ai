...
def _definition() -> dict:
    section = TemplateSection(
        id="agenda",
        name="Agenda",
        voice_aliases=("порядок денний",),
        required=True,
        field_type=FieldType.FREE_TEXT,
        asr_prompt="порядок денний зустрічі",
        min_chars=0,
    )
...
def test_create_template_returns_id_and_audits(
    ...
    assert captured["definition"].code == "meeting_notes"

    calls = client.audit_calls  # type: ignore[attr-defined]
    assert len(calls) == 1
    assert calls[0]["kind"] == "template.created"
    assert calls[0]["payload"] == {"code": "meeting_notes", "category": "meetings"}
