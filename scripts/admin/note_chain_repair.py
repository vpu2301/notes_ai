...
async def dump(dsn: str, note_id: UUID) -> int:
    conn = await asyncpg.connect(dsn)
    try:
        meta = await conn.fetchrow(
            "SELECT tenant_id, code, status, current_version_id FROM notes WHERE id = $1",
            note_id,
        )
        if meta is None:
        ...
        failures = await conn.fetch(
            ...
            FROM audit.note_chain_failures
            WHERE note_id = $1
...
def main() -> int:
    ...
    parser.add_argument("--note-id", required=True, type=UUID)
    parser.add_argument(
        "--dsn",
    ...
    return asyncio.run(dump(args.dsn, args.note_id))
