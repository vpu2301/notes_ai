...
def test_multi_choice_valid() -> None:
    meta = parse_field_metadata(
        "multi_choice",
        {"selected": ["email", "phone"], "confidence": 0.85, "source": "extracted"},
    )
    assert isinstance(meta, MultiChoiceMeta)
    assert meta.selected == ("email", "phone")
...
def test_multi_choice_duplicate_values_rejected() -> None:
    with pytest.raises(FieldMetadataError, match="unique"):
        parse_field_metadata("multi_choice", {"selected": ["email", "email"], "source": "manual"})


def test_multi_choice_scalar_selected_rejected() -> None:
    with pytest.raises(FieldMetadataError):
        parse_field_metadata("multi_choice", {"selected": "email", "source": "manual"})
...
def test_numeric_valid() -> None:
    meta = parse_field_metadata(
        "numeric_with_unit",
        {"value": 25000.0, "unit": "USD", "confidence": 0.95, "source": "extracted"},
    )
    assert isinstance(meta, NumericMeta)
    assert meta.value == 25000.0


def test_numeric_missing_unit_rejected() -> None:
    with pytest.raises(FieldMetadataError):
        parse_field_metadata(
            "numeric_with_unit", {"value": 25000.0, "confidence": 0.95, "source": "extracted"}
        )
...
def test_numeric_non_number_rejected() -> None:
    with pytest.raises(FieldMetadataError):
        parse_field_metadata(
            "numeric_with_unit",
            {"value": "twenty five", "unit": "USD", "confidence": 0.9, "source": "extracted"},
        )
