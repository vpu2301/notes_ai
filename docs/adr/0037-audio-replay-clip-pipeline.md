...
Sprint-08 reserved `NoteSection.transcript_segment_ids` "for sprint-15
note review"; per-word timings exist since sprint 04. A user
...
- note-service owns every note-read control (purpose gate,
  `note.read`) but had no S3/crypto wiring; dictation-
  service has the wiring but is a capacity-constrained inference fleet.

## Decision

**Clip-on-demand in note-service**: fetch encrypted object → decrypt
...
`mdx-audio-clip-v1`, bound to tenant+clip, 5-min TTL) redeemed at
authenticated
`GET /v1/audio-clips/{clip_id}?t=…` which decrypts and streams
`audio/ogg`.

- **Wire deviation from the spec**: `POST /v1/audio-clips` takes
  `{note_id, start_ms, end_ms}`, not `session_or_audio_ref` — the
  purpose gate and author check are note-anchored; a raw session ref
  would bypass both. The server resolves `source_session_id` /
  `source_asr_job_id` → `audio_files`.
- **Ephemeral derivatives, never a second copy of sensitive content**:
  Redis registry (`SETEX 300`) is the real lifetime; 1-day bucket ILM is
  the ciphertext backstop (mc has no sub-day granularity); the
  tenant-KEK envelope means key destruction crypto-shreds clips with
  everything else. No new table → nothing extra to track or export.
...
- **Segment listing degrades honestly**: sections with
  ...
  `speaker_role` labels per segment); everything older returns the whole
...
- Caps: span ≤ 60 s, 30 clips/user/hour (Redis fixed window, fail-open)
  ...
  (`note.audio_replayed`).
- Cross-service reads: note-service reads `dictation_sessions` /
...
- note-service gains ffmpeg + S3/crypto wiring (Dockerfile, compose,
  master-key mount) — env names identical to dictation-service.
- Back-filling `transcript_segment_ids` on finalized versions is
  impossible without an amendment (it sits inside the JCS-hashed bytes);
  the whole-transcript fallback is the permanent answer for old notes.
