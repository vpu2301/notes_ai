...
/// Upcoming events from the phone's calendars (EventKit), for the home
/// page's "Coming up" list. Read-only; access is asked for once.
///
/// Google, Outlook and iCloud calendars all arrive through the same store
/// once their account is added in Settings › Apps › Calendar › Accounts;
/// the Connectors page lets the user pick which calendars feed the list.
@MainActor
final class CalendarService: ObservableObject {
    ...
    /// One calendar as shown in the Connectors page.
    struct CalendarInfo: Identifiable, Equatable {
        let id: String
        let title: String
        /// The account it belongs to ("iCloud", "Google", "Exchange", "On My iPhone").
        let source: String
        let color: CGColor?
    }

    enum Access: Equatable {
        case unavailable   // no usage description in the bundle
        case notAsked
        case denied
        case granted
    }
    ...
    init() {
        ...
        // Calendars come and go (an account added in Settings, an event
        // moved in the Calendar app): keep the list current.
    ...
    /// switch in Settings while the app was in the background).
    func recheckAccess() {
