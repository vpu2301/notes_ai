...
final class NoteViewModel: ObservableObject {
    ...
    /// Set when an export is ready; the view presents the share sheet.
    ...
    /// Write the pending content now (also called before finalize and when
    /// the page goes away).
    func flush() async {
        saveTask?.cancel()
    ...
    /// Fetch the PDF and hand it to the share sheet (Files, Mail, AirDrop…).
    func exportPDF() async {
        ...
        do {
            ...
            shareItem = ShareItem(url: try Self.exportFile(named: "\(note?.code ?? "note").pdf", data: data))
        } catch {
            actionError = error.localizedDescription
        }
    }
    ...
    func exportMarkdown() {
        do {
            shareItem = ShareItem(url: try Self.exportFile(named: "\(note?.code ?? "note").md",
