...
struct ConnectorsView: View {
    ...
    private var calendarAction: some View {
        ...
        case .denied:
            Button("System Settings") { open("x-apple.systempreferences:com.apple.preference.security?Privacy_Calendars") }
    ...
    private var calendarList: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if calendar.calendars.isEmpty {
                Text("No calendars yet — add an account below.")
                    .font(.dsMeta)
                    .foregroundStyle(DS.muted)
                    .padding(.top, 8)
            }
            ...
            HStack(spacing: 8) {
                ...
                Text("Google or Outlook calendar missing? Add the account in System Settings; it shows up here.")
                    .font(.dsMeta)
                    .foregroundStyle(DS.text3)
                    .fixedSize(horizontal: false, vertical: true)
                Spacer(minLength: 6)
                Button("Internet Accounts") { open("x-apple.systempreferences:com.apple.Internet-Accounts-Settings.extension") }
    ...
    private var connectorsCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                DSLabel("Connectors")
                Spacer()
                DSMenu(width: 260, items: presetItems) {
                    HStack(spacing: 5) {
            ...
            if store.connectors.isEmpty {
                Text("Connect the tools your notes should reach — your CRM, wiki or tracker — through their MCP servers. Sign-ins stay in this Mac's Keychain.")
                    .font(.dsMeta)
                    .foregroundStyle(DS.muted)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func menuItems(for connector: Connector) -> [DSMenuItem] {
        ...
        if !connector.toolNames.isEmpty {
            items.append(.item("Copy tool list", symbol: "doc.on.doc") {
                ...
                NSPasteboard.general.setString(connector.toolNames.joined(separator: "\n"), forType: .string)
            })
        }
...
private struct ConnectorRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            .help(connector.toolNames.isEmpty ? connector.url : connector.toolNames.joined(separator: ", "))
            Spacer(minLength: 8)
            chip
            action
            DSMenu(width: 200, items: menu) { DSMoreLabel(dim: true) }
        }
        .padding(.vertical, 4)
    }
...
private struct ConnectorEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text(connector.url.isEmpty ? "New connector" : "Edit connector")
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                field("Server URL") { DSTextField(placeholder: "https://…/mcp", text: $url, mono: true) }
                field("Sign in with") {
                    DSSelect(options: [
                ...
                if auth == .token {
                    ...
                    Text("A private-app token or API key, sent as a bearer token. Stored in this Mac's Keychain.")
                        .font(.dsMeta)
                        .foregroundStyle(DS.muted)
                        .fixedSize(horizontal: false, vertical: true)
                } else if auth == .oauth {
                    Text("The server's own sign-in page opens in your browser; the app registers itself and keeps the token in the Keychain.")
                        .font(.dsMeta)
                        .foregroundStyle(DS.muted)
                        .fixedSize(horizontal: false, vertical: true)
                    DSDivider()
                    Text("Some servers (HubSpot) do not register apps themselves. Create an app on their side with this redirect URL and paste its credentials:")
                        ...
                        .fixedSize(horizontal: false, vertical: true)
                    HStack(spacing: 6) {
                        Text(MCPOAuth.loopbackRedirectURI)
                            .font(.dsMono(11.5))
                            .foregroundStyle(DS.text2)
                            .textSelection(.enabled)
                        Button {
                            NSPasteboard.general.clearContents()
                            NSPasteboard.general.setString(MCPOAuth.loopbackRedirectURI, forType: .string)
                        } label: {
                    ...
                    field("Client ID (optional)") { DSTextField(placeholder: "From the server's app settings", text: $clientId, mono: true) }
