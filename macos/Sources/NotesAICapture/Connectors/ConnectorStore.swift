...
/// Generic-password items in this Mac's login Keychain.
///
/// Two services use it: connector tokens (the default) and, since IDX-M1,
/// the sign-in session (`SessionStore`). Both are device-only, never
/// synchronised to iCloud, and readable only after the first unlock — a
/// refresh token that syncs is a refresh token on every Mac the person
/// has ever owned.
///
/// Every item is keyed to the app's code-signing identity, which is why
/// `scripts/make-app.sh` signs with a persistent one: an ad-hoc signature
/// changes on every build and would drop the session exactly as it used
/// to drop the microphone grant.
enum Keychain {
